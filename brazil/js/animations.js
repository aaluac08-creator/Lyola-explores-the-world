...
function animateScrollSetups(configs = []) {
...
function initLegacyAnimations() {
    ...
    const animateElements = (selector, vars) => {
        const elements = legacySection.querySelectorAll(selector);
        if (!elements.length) {
            return;
        }
    ...
    animateElements('.insight-card', {});
    animateElements('.food-story', {y: 60});
}

function initGeneralSectionAnimations() {
    animateScrollSetups([
        { selector: '.race-bar', trigger: '.race-bar-block', y: 25, stagger: 0.1 },
        { selector: '.race-note', trigger: '.race-note', y: 20, stagger: 0 },
        { selector: '.geography-summary .geo-card', trigger: '.geography-summary', y: 40, stagger: 0.2 },
        { selector: '.quirks-section .quirk-card', trigger: '.quirks-grid', y: 40, stagger: 0.15 },
        { selector: '.outro-copy p', trigger: '.outro-copy', y: 30, stagger: 0.1, fade: false },
        { selector: '.outro-contact', trigger: '.outro-copy', y: 20, stagger: 0 },
    ]);
}
